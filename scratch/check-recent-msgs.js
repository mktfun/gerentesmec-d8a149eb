...
async function checkRecentMsgs() {
  ...
  const { data: leads } = await supabase.from('leads').select('id, customer_name, chatwoot_conversation_id, created_at, funnel_stage').order('created_at', { ascending: false }).limit(5);
  console.log(leads);

  console.log("\nChecking last 5 messages...");
  const { data: msgs } = await supabase.from('chat_messages').select('id, lead_id, content, chatwoot_message_id, created_at').order('created_at', { ascending: false }).limit(5);
  console.log(msgs);
}
