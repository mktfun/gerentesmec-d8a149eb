...
const Relatorios = () => {
  ...
  const handleCleanDB = async () => {
    if (!window.confirm("Atenção: Isso irá remover os arquivos de mídia (vídeos, áudios e imagens) de mensagens com mais de 7 dias para liberar espaço no banco de dados. Essa ação não pode ser desfeita. Deseja continuar?")) return;
    
    setIsCleaningDB(true);
    try {
      const { error } = await supabase.rpc('clean_old_media');
      if (error) throw error;
      alert("Limpeza concluída com sucesso! Espaço liberado.");
    } catch (err: any) {
      console.error(err);
      alert("Erro ao limpar mídias: " + err.message);
    } finally {
