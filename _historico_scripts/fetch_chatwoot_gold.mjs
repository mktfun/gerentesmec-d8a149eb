...
const BLACKLIST = ['mecânico', 'mecanico', 'gerente', 'gerência', 'gerencia', 'peças', 'pecas', 'fornecedor', 'auto peças', 'oficina'];
...
async function run() {
  ...
  for (const inbox of inboxes) {
    ...
    for (const c of convs) {
      ...
      // Filtro heurístico: Cliente tem que falar no min 3 vezes e oficina no min 4 vezes
      if (clienteMsgs.length >= 3 && oficinaMsgs.length >= 4) {
        // Verifica se em alguma mensagem tem "orçamento", "valor", "aprovação" ou "ok" (indício de cliente fechando negócio)
        const hasBusinessTalk = cleanMsgs.some(m => m.content && (m.content.toLowerCase().includes("orçamento") || m.content.toLowerCase().includes("valor") || m.content.toLowerCase().includes("aprova") || m.content.toLowerCase().includes("ok") || m.content.toLowerCase().includes("link")));
        ...
        if (hasBusinessTalk) {
            ...
            console.log(`Achei conversa GOLD. ID: ${c.id} - Cliente: ${senderName}`);
        }
      }
    }
  ...
  fs.writeFileSync('chatwoot_june_gold.json', JSON.stringify(result, null, 2));
  console.log('Done! Saved to chatwoot_june_gold.json');
}
