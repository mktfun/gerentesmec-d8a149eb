...
const files = ['Relatorio_Semantico_DOM_PEDRO.html', 'Relatorio_Semantico_Jorge_Beretta.html'];
...
for (const f of files) {
    ...
    // Check if already is matrix
    if(html.includes('Tabela Analítica de Critérios')) {
        console.log('Already matrix: ' + f);
        continue;
    }
    ...
    if(diagMatch) diagnostico = diagMatch[1];

    const initialCardHeaderStart = html.indexOf('<div class="conv-card"');
    
    let clientRegex = /<div class="conv-card[^>]*>[\s\S]*?<span class="client-info">Cliente: (.*?)<\/span>[\s\S]*?<\/table>[\s\S]*?<\/div>/gi;
    let match;
    let totalAcertos = 0;
    
    while ((match = clientRegex.exec(html)) !== null) {
        let clientName = match[1].trim();
        let clientHtml = match[0];
        ...
        rules.forEach(rule => {
            ...
            if (ruleMatch) {
                const passed = ruleMatch[1].toLowerCase() === 'yes';
    ...
    if(clients.length === 0) {
        console.log('No clients found in ' + f);
        continue;
    }
...
${clientCols}                        <td style="text-align:center; font-weight:bold; background:#f8fafc; border-bottom: 1px solid var(--border);"><span style="color:${perfColor};">${perfPercent}%</span> (${simCount}/${clients.length})</td>
    ...
    // Achar onde o primeiro cliente comeca para substituir o dossie atual
    let firstClientMarker = html.indexOf('<div class="conv-card"');
    if(firstClientMarker !== -1) {
       firstClientMarker = html.indexOf('<div class="conv-card"', firstClientMarker + 10); // O segundo (primeiro cliente)
       if(html.indexOf('<!-- Cliente') !== -1) {
           firstClientMarker = html.indexOf('<!-- Cliente');
       }
    }

    let replacedHtml = html.substring(0, initialCardHeaderStart) + newDossie + '\n    ' + html.substring(firstClientMarker);
    
    // Ajustar o CSS para 1 PAGINA EXATA por auditoria (avoid internal break e break before)
    ...
    if(cssMatch) {
        ...
        replacedHtml = replacedHtml.replace('</style>', newPrintCss + '\n</style>');
    }

    fs.writeFileSync(fp, replacedHtml);
    console.log('Fixed matrix for: ' + f);
}
